...
window.BrandKitStore = (function () {
  ...
  const EMPTY_KIT = { logo_url: null, colours: {}, fonts: {} };
  ...
  async function getBrandKit() {
    ...
    return { logo_url: data.logo_url, colours: data.colours || {}, fonts: data.fonts || {}, scoped: true, id: data.id };
  }

  /**
   * Upserts colours/fonts (never logo_url — see uploadLogo). Creates the
   * row if this scope has none yet.
   * @param {{colours?: object, fonts?: object}} fields
   */
  ...
  return { getBrandKit, saveBrandKit, uploadLogo, getScope };
})();
