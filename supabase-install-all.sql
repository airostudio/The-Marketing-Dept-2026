...
--
-- A customer replying to a resolved ticket reopens it: from their side they
-- are still having the problem, and a ticket that stays closed while the
-- customer is still writing into it is a ticket nobody will look at again.
...
BEGIN
  ...
  UPDATE support_tickets
     SET last_reply_at = NEW.created_at,
         ...
         status = CASE
           WHEN NEW.author_role = 'customer' AND status IN ('resolved','closed') THEN 'open'
